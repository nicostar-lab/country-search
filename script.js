...
async function getCountry(withIp = true) {
	let country;
	if (withIp) {
		...
		country = await fetch(`https://api.ipapi.com/${ip}?access_key=c20f1ab92ef0e1ce9d5e1ba9a9616ce2`)
			.then(response => response.json())
			.then(json => {
				const countryName = json.country_name;

				for (let i = 0; i < countries_data.length; i++) {
					if (countries_data[i].name.toLowerCase() === countryName.toLowerCase()) {
						name.textContent = countries_data[i].name;
						capital.textContent = countries_data[i].capital || 'No capital';
						population.textContent = countries_data[i].population + ' habitants';
						flag.src = countries_data[i].flag;
					}
				}
			}
			);
		...
		for (let i = 0; i < countries_data.length; i++) {
			if (countries_data[i].name.toLowerCase() === country.toLowerCase()) {
